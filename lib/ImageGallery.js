...
class ImageGallery extends React.Component {
  ...
  shouldComponentUpdate(nextProps) {
    return !shallowEqualsIgnoreKeys(this.props, nextProps, [
  ...
  render() {
    ...
    * the tabBarHeight? */
    ...
    const topClippingOffset = isDragging
      ? 0
      : TOP_OFFSET - Layout.statusBarHeight;
    const bottomClippingOffset = isDragging
      ? -topClippingOffset
      : -topClippingOffset +
        Layout.tabBarHeight +
        SOME_NUMBER_I_DONT_UNDERSTAND -
    ...
    return (
      <View
        ...
        <Animated.View
          style={[
            styles.underlay,
            { opacity: zoomInOutValue, top: isDragging ? 0 : undefined },
          ]}
        />
        ...
        <View
          ...
          <View
            style={[
              styles.fullSize,
              { transform: [{ translateY: isDragging ? 0 : -TOP_OFFSET }] },
            ]}>
  ...
  _animateZoomIn = (props = this.props) => {
    ...
    setTimeout(() => {
      Animated.parallel([
        ...
        requestAnimationFrame(() => {
          this.props.dispatch(
            Actions.updateImageGalleryLifecycle(OPEN_AND_IDLE)
          );
        });
      });
    });
  ...
  _isAnimatedImageVisible() {
    return (
      [CLOSED_AND_IDLE, OPEN_AND_IDLE].indexOf(this.props.lifecycle) === -1
    );
  }
  ...
  _onPanResponderMove = (e, { dy }) => {
    if (this._responderGranted) {
      ...
      this.props.dispatch(
        Actions.updateImageGalleryLifecycle(DRAG_IN_PROGRESS)
      );
  ...
  _onPanResponderEnd = (e, { moveX, moveY, dx, dy, vx, vy }) => {
    ...
    if (
      Platform.OS === 'android' &&
      (dy >= throwThreshold || dy <= -throwThreshold)
    ) {
      shouldAnimateOut = true;
    } else if (
      Platform.OS === 'ios' &&
      (vy >= 1 || vy <= -1 || (dy >= throwThreshold || dy <= -throwThreshold))
    ) {
      shouldAnimateOut = true;
    }
    ...
    if (shouldAnimateOut) {
      ...
      Animated.parallel([
        Animated.timing(innerContentValue, {
          toValue: 1,
          duration,
          useNativeDriver: USE_NATIVE_DRIVER,
        }),
        Animated.timing(zoomInOutValue, {
          toValue: 1,
          duration,
          useNativeDriver: USE_NATIVE_DRIVER,
        }),
